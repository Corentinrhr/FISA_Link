...
function insererQRCodes() {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            // Si le conteneur existe, vider son contenu actuel
            if (container) {
                container.innerHTML = '';  // Supprimer le contenu existant
                ...
                // Itérer sur chaque service pour créer les éléments d'images et de liens
                services.forEach(service => {
                    ...
                    if (service.name === "Paylib") {
                        ...
                        linkElement.textContent = data.paylib_number;
                    } else {
                        linkElement.href = service.link;
                        linkElement.textContent = 'Payer avec ' + service.name;
                    }
                    linkElement.classList.add('qr-code-link');
                    
                    // Ajouter l'image et le lien au conteneur
                    const itemContainer = document.createElement('div');
                    itemContainer.classList.add('qr-code-item');
                    itemContainer.appendChild(imgElement);
                    itemContainer.appendChild(linkElement);

                    container.appendChild(itemContainer);  // Ajouter l'item au conteneur existant
                });
            } else {
